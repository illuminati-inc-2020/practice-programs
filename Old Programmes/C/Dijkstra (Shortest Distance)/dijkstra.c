...
int dijkstra(int G[10][10],int v,int v1,int v2);

int main(int argc,char* argv[])
{
	int i,j,v,v1,v2,G[10][10];
...
int dijkstra(int G[10][10],int v,int v1,int v2)
{
	...
	while(S[scount]!=v2)
	{
						//Determining shortest path found till now
		int min=INF;
		scount++;
		for(i=0;i<v;i++)
			if(!belongs(S,scount-2,i) && P[i]<min)
			{
				...
				S[scount]=i;
						...
						//Updating P[] with new vertex
		for(i=0;i<v;i++)
			if(!belongs(S,scount,i) && P[i]>min+G[S[scount]][i])
