 ...
 *	Version 2.0
 *	(This version uses the buffer for positioning the cursor)
 */
					...
					for(i=0;i<n && c!='C';i++)
					{
						...
						switch(c)
						{
							case 'Y':
								...
								{
									show_messege("File write error",1);
									c='C';
								}
								break;
							case 'C':
...
int open_window(fwin *fw)
{
	...
	{
		erase();
		mvprintw(0,0,"| F1 Help | F2 New | F3 Open | F5 Save | F6 Save as | Esc Minimize window | Cntrl+X Close window |");
		refresh();
		werase(fw->win);
		mvwprintw(fw->win,0,0,"%s",fw->name);
