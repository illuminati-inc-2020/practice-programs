...
long long int diff(date d1,date d2,date *d3)
{
	...
	char i,days[48]={31,29,31,30,31,30,31,31,30,31,30,31,31,28,31,30,31,30,31,31,30,31,30,31,31,28,31,30,31,30,31,31,30,31,30,31,31,28,31,30,31,30,31,31,30,31,30,31};
	date temp;
	d3->dd=d3->mm=d3->yyyy=0;
	for(copy_date(&temp,d1);comp_date(temp,d2)!=0;incr_date(&temp))	x++;
	for(i=((d1.yyyy%4)*12+d1.mm-1)%48,_x=x;_x>=days[i];i=(i+1)%48)
	{
		_x-=days[i];
		d3->mm++;
		if(d3->mm==12)
