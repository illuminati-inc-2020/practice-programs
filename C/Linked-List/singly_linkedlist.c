	...
	while(choice2!=1)
	{
		...
		switch(choice)
		{
			...
			case 13:  //Swap adjacent nodes
				printf("Enter the index of the first data to be swapped :   ");
				scanf("%d",&idx);
				if(idx<1 || idx>=max)
					if(max<=1)
					{
						printf("There's too few element to be swapped");
						getch();
					}
					else
					{
						printf("Wrong Index, you can enter only index %s%d",(max==2)?"":"between 1 and ",max-1);
						getch();
					}
				else
			...
			case 15:  //Clear linked list
				printf("Enter index of linked list you want to clear :   ");
				scanf("%d",&idx);
			...
			case 17:  //Search and insert after a data
				...
				break;
			case 18:  //Merge
				printf("Enter index of the first linked list :   ");
				scanf("%d",&idx);
				printf("Enter index of the second linked list :   ");
				scanf("%d",&idx2);
				if(idx>=maxlist || idx<0 || idx2>=maxlist || idx2<0 || idx==idx2)
				{
					printf("Wrong index or same index");
					getch();
				}
				else
					merge(x+idx,x+idx2);
