...
void keep_action_record(fwin*,int,char*,int,char*,int);
					//Pushes an action to a linked list
action* flush_list(action*);		//Flushes a linked list of actions
...
int main(int argc,char* argv[])	
{
	...
	if(argc>1)
	{
		extern int load_file(fwin*,char*);
		for(i=2;i<=argc;i++)
		{
			current++;
			wn++;
			w[wn-1]=create_win(w[wn-1],argv[i-1]);
			load_file(w[wn-1],argv[i-1]);
		}
...
{
	fw->redo=flush_list(fw->redo);
	fw->undo=flush_list(fw->undo);
...
int open_window(fwin *fw)
{
	...
	while(1)
	{
		...
		werase(head);
		for(i=0;i<cols;i++) waddch(head,'_');
		...
		if(isprint(c) || isspace(c))	
		{
			if(!replace || (replace && fw->buffer[fw->bufpos]=='\n'))
			{
				...
				{
					keep_action_record(fw,fw->hstart,fw->buffer+fw->hstart,fw->hend-fw->hstart,(char*)&c,1);
					memmove(fw->buffer+fw->hstart,fw->buffer+fw->hend,BUFFER_SIZE-fw->hend);
					fw->bufpos=fw->hstart;
				}
				else keep_action_record(fw,fw->bufpos,"",0,(char*)&c,1);
				memmove(fw->buffer+fw->bufpos+1,fw->buffer+fw->bufpos,bufsize-fw->bufpos);
			}
			else keep_action_record(fw,fw->bufpos,fw->buffer+fw->bufpos,1,(char*)&c,1);
		...
		else			
		{
			...
			
			{
				...
				case KEY_DC:		//Delete  --  Delete character or string at cursor
					...
					if(fw->buffer[fw->hend+1]=='\0') 
						memset(fw->buffer+fw->hstart,0,BUFFER_SIZE-fw->hstart);
					if(fw->hend==fw->hstart)
				...
				case KEY_F(8):		//F8  --  Paste
					...
					memmove(fw->buffer+fw->bufpos+strlen(copy_buffer),
							fw->buffer+fw->bufpos, BUFFER_SIZE-fw->bufpos-strlen(copy_buffer));
...
void instructions()
{
	...
	mvwaddstr(win,18,59,"Alpha v4.0");
...
int load(fwin* fw)
{
	int load_file(fwin* fw,char* filename);
	char filename[256];			//holds file name
	show_messege("Enter file name to be loaded : ",2,0,"%s",filename);
	int flag=load_file(fw,filename);
	if(flag==1)
...
void redo(fwin* fw)
{
	...
	{
										//redoing
		int i,len=strlen(fw->redo->ins);
		fw->bufpos=fw->redo->bufpos;
		memmove(fw->buffer+fw->bufpos+len,
				fw->buffer+fw->bufpos+strlen(fw->redo->del),strlen(fw->buffer)-fw->bufpos);
		for(i=0;i<len;i++)
			fw->buffer[fw->bufpos++]=fw->redo->ins[i];
										//pushing action in undo queue
		action* a;
		a=fw->redo;
		fw->redo=fw->redo->next;
		a->next=fw->undo;
		fw->undo=a;
	}
}
...
void keep_action_record(fwin* fw,int bufpos,char* del,int dellen,char* ins,int inslen)
{
	...
	a->bufpos=bufpos;
	a->del=(char*)malloc(dellen);
	strncpy(a->del,del,dellen);
	a->del[dellen]='\0';
	a->ins=(char*)malloc(inslen);
	strncpy(a->ins,ins,inslen);
	a->ins[inslen]='\0';
	a->next=fw->undo;
	fw->undo=a;
	fw->redo=flush_list(fw->redo);
