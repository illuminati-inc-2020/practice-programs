 ...
 *	Version 3.1
 *	(This version includes search and replace)
 */
		...
		switch(getch())
		{
			...
			case 24:		//Cntrl+X	--	Exit
				for(i=0;i<n && flag==0;i++)
				{
					bkgd(COLOR_PAIR(1));
					refresh();
					show_messege("do you want to save %s\n\t\tyes(y)/no(n)/cancel(c)",1,1,w[i]->name,&c);
					switch(toupper(c))
					{
						case 'Y':
							if(save(w[i],0))
...
{
	int i,n;
	for(i=0,n=0;fw->buffer[i] && i<fw->bufpos;i++) if(fw->buffer[i]=='\n') n++;
	return n;
}
...
int open_window(fwin *fw)
{
	...
	{
		erase();
		mvprintw(0,0,"| F4 Save | F5 Save as | Esc Minimize | Cntrl+X Close | F1 Help |");
		mvchgat(0,0,COLS,A_NORMAL|A_UNDERLINE,1,NULL);
		refresh();
...
int save(fwin *fw,char save_as)
{
	...
	if(save_as || !fw->saved) show_messege("Enter file name to be loaded : ",2,0,"%s",filename);
