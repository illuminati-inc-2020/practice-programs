...
int main()
{
	...
	while((c=getc(fp))!=EOF)
	{
		...
		if(slc==0 && mlc==0)
		{
			...
			if(top2<=-1)
			{
				if(isalnum_(cp) && isspace(c) && isoperator(cn))
					continue;
				if(isoperator(cp) && isspace(c) && isalnum_(cn))
					continue;
				if(isspace(cp) && isspace(c))
					continue;
				if(cp==')' && isspace(c) && cn==';')
					continue;
...
int differ(char a,char b)
{
	if( (isalnum_(a) && (isoperator(b) && b!=',')) || (isoperator(a) && isalnum_(b)))
	  	return 1;
